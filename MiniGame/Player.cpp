...
HRESULT PLAYER::Initialize(int num)
{
	...
	{
		Pos[i].x = PLAYER_WIDTH / 2.0f;
		Pos[i].y = SCREEN_HEIGHT - PLAYER_HEIGHT / 2.0f - (Interval * num);
	}
	...
	for (nCounter = 0; nCounter < 4; nCounter++)
	{
		...
		pVertex[nCounter].Position.x = pVertex[nCounter].U * PLAYER_WIDTH + Pos[num].x;
		pVertex[nCounter].Position.y = pVertex[nCounter].V * PLAYER_HEIGHT + Pos[num].y;
...
void PLAYER::Update(int num)
{
	...
	{//無敵位置（スタート位置）へ
		Pos[num].x = PLAYER_WIDTH / 2.0f;
		Pos[num].y = SCREEN_HEIGHT - PLAYER_HEIGHT / 2.0f - (Interval * num);
	}

	//----- マウスカーソルとの当たり判定 -----//
	if (INPUT_MANAGER::GetMouseButton(BUTTON_LEFT, HOLD) &&
		(Pos[num].x - MousePos->x) * (Pos[num].x - MousePos->x) + (Pos[num].y - MousePos->y) * (Pos[num].y - MousePos->y) <= 
		(PLAYER_RADIUS + 20) * (PLAYER_RADIUS + 20))
	{
		//ポジションの更新
		Pos[num].x = MousePos->x;
		Pos[num].y = MousePos->y;
		//重力の無効
		Move.y = 0.0f;
		//掴まれたアニメーション
		Anim = 1;
	...
	{
		//無敵位置（スタート位置）判定
		if (Pos[num].x - PLAYER_WIDTH < 50.0f)
		{
			Invincible = true;
		}
		//通常アニメーション
	...
	for (int i = 0; i < 4; ++i)
	{
		pVertex[i].Position.x = Pos[num].x + (i & 1) * PLAYER_WIDTH - PLAYER_WIDTH / 2;
		pVertex[i].Position.y = Pos[num].y + (i >> 1) * PLAYER_HEIGHT - PLAYER_HEIGHT / 2;
	}
	...
	//----- 移動範囲制限 -----//
	if (Pos[num].x + PLAYER_WIDTH / 2.0f > SCREEN_WIDTH)			//右端
	{
		Pos[num].x = SCREEN_WIDTH - PLAYER_WIDTH / 2.0f;
	}
	else if (Pos[num].x - PLAYER_WIDTH / 2.0f < 0)				//左端
	{
		Pos[num].x = PLAYER_WIDTH / 2.0f;
	}
	if (Pos[num].y > SCREEN_HEIGHT - PLAYER_HEIGHT / 2)			//下端
	{
		Pos[num].y = SCREEN_HEIGHT - PLAYER_HEIGHT / 2;
		Move.y = 0.0f;
	}
}
